...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/piratebootyd"
  then
    piratebootyd="$dir/piratebootyd"
    break
  fi
done
if test -z "$piratebootyd"
then
  echo "piratebootyd not found"
  exit 1
fi
echo "Found: $piratebootyd"

TORDIR="$DIR/piratebooty-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting piratebootyd"
  tail -n 400 "$HOME/.piratebooty/piratebooty.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Pirate Booty\ \'" | tail -n 20
  exit 1
fi
